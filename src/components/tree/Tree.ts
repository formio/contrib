...
const Component = (Components.components as any).component;
...
export default class TreeComponent extends NestedDataComponent {
  ...
  createComponents(data, node?) {
    const components = this.componentComponents.map(
      (component) => {
        const componentInstance: any = Components.create(component, this.componentOptions, data);
  ...
  setRoot() {
    ...
    this.treeRoot = new Node(null, value, {
      ...
      removeComponents: this.removeComponents as any,
