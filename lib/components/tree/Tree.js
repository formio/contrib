...
const Tree_form_1 = require("./Tree.form");
const NestedDataComponent = js_1.Components.components.nesteddata;
const Component = js_1.Components.components.component;
class TreeComponent extends NestedDataComponent {
    ...
    createComponents(data, node) {
        const components = this.componentComponents.map((component) => {
            const componentInstance = js_1.Components.create(component, this.componentOptions, data);
    ...
    attach(element) {
        ...
        return native_promise_only_1.default.all([
    ...
    attachNode(element, node) {
        ...
        let componentsPromise = native_promise_only_1.default.resolve();
        let childrenPromise = native_promise_only_1.default.resolve();
    ...
    attachComponents(node, ...args) {
        ...
        return node.refs.nodeEdit
            ...
            : native_promise_only_1.default.resolve();
    }
    attachChildren(node) {
        const childElements = node.refs.childNodes.children;
        return native_promise_only_1.default.all(lodash_1.default.map(childElements, (childElement, index) => this.attachNode(childElement, node.children[index])));
    }
    ...
    cancelNode(node) {
        ...
        this.hook('tree.cancelNode', {
            ...
            if (node.isRoot) {
                if (node.persistentData && !lodash_1.default.isEmpty(node.persistentData)) {
                    node.cancel();
                    this.redraw();
                }
                else {
    ...
    setRoot() {
        ...
        this.treeRoot = new Node_1.default(null, value, {
    ...
    checkNode(data, node, flags, row) {
        return node.children.reduce((result, child) => this.checkNode(data, child, flags, row) && result, super.checkData(data, flags, node.data, node.components) && !node.editing && !node.new);
    }
    getComponents() {
...
TreeComponent.editForm = Tree_form_1.default;
