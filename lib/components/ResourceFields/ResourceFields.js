...
const NestedComponent = formiojs_1.Components.components.nested;
class ResourceFieldsComponent extends NestedComponent {
    ...
    addDynamicFields() {
        ...
        Formio.request(`${Formio.getProjectUrl()}/form/${this.data.resource}`).then((result) => {
            const dynamicFields = this.root.getComponent('dynamic');
            dynamicFields.destroyComponents();
            const formFields = [{
                    value: 'data',
                    label: 'Entire Submission Data',
                }];
            formiojs_1.Utils.eachComponent(this.options.currentForm.components, (component, path) => {
            ...
            formiojs_1.Utils.eachComponent(result.components, (component, path) => {
                if (component.type !== 'button' && path.indexOf('.') === -1) {
...
exports.default = ResourceFieldsComponent;
