...
const { _ } = js_1.Utils;
const { get } = _;
const OriginalWizard = js_1.Displays.displays.wizard;
class Wizard extends OriginalWizard {
    ...
    hasButton(name, nextPage = this.getNextPage()) {
        ...
        const { previous = this.options.buttonSettings.showPrevious, cancel = this.options.buttonSettings.showCancel, submit = this.options.buttonSettings.showSubmit, next = this.options.buttonSettings.showNext } = get(this.currentPage, 'component.buttonSettings', {});
        const dynamicWizardComponent = this.hasDynamicWizard();
    ...
    setChangingMode() {
        ...
        if (dynamicWizard && !this.editMode && dynamicWizard.hasComponents) {
            ...
            if (this.element && !this.element.classList.contains('dynamicWizard-changingMode')) {
                ...
                if (dynamicWizard?.editRows.length === get(dynamicWizard?.component, 'validate.maxLength')
